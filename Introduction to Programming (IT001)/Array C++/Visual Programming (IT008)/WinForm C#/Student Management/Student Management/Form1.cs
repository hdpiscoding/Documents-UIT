...
namespace BT6._1
{
    public partial class Form1 : Form
    {
        ...
        static String connString = @"Data Source=BUDDY\SQLEXPRESS;Initial Catalog=STUMANAGEMENT;Integrated Security=True;MultipleActiveResultSets=True";
        ...
        {
            String sqlQuery = "select * from [STUMANAGEMENT].[dbo].[SINHVIEN]";
        ...
        private void insert()
        {
            String sqlQuery = "insert into [STUMANAGEMENT].[dbo].[SINHVIEN] values (@ID, @HOTEN, @NGAYSINH)";
            SqlCommand command = new SqlCommand(sqlQuery, connection);
            command.Parameters.Add("@ID", SqlDbType.VarChar, 3).Value = txt_masv.Text.ToString();
            command.Parameters.Add("@HOTEN", SqlDbType.VarChar, 30).Value = txt_ten.Text.ToString();
            command.Parameters.Add("@NGAYSINH", SqlDbType.DateTime).Value = txt_ngsinh.Text.ToString();
            try
            {
        ...
        private void delete()
        {
            ...
            String sqlQuery = "delete from [STUMANAGEMENT].[dbo].[SINHVIEN] where ID = '" + id + "'";
            SqlCommand command = new SqlCommand(sqlQuery, connection);
            try
            {
                command.ExecuteNonQuery();
        ...
        private void update()
        {
            int CurrentIndex = dataGridView1.CurrentRow.Index;
            string id = dataGridView1.Rows[CurrentIndex].Cells[0].Value.ToString();
            String sqlQuery = "update [STUMANAGEMENT].[dbo].[SINHVIEN] set HOTEN = '" + txt_ten.Text.ToString() + "', NGAYSINH = '" + txt_ngsinh.Text.ToString() +
                "' where ID = '" + id + "'";
            SqlCommand command = new SqlCommand(sqlQuery, connection);
